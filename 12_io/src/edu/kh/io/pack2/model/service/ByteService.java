...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		try {
			...
			*/
			...
			System.out.println("[수행 시간] : "+(endTime - startTime+"ns")); // 63900ns
	...
	public void bufferedFileByteOutput() {
		 ...
		 * (내부 버퍼에 데이터를 모아두었다가 일정 크기가 되면 한 번에 출력)
		...
		try {
			...
			System.out.println("[수행 시간] : "+(endTime - startTime+"ns")); // 36901ns
	...
	public void bufferedFileByteInput() {
		...
		try {
			...
			e.printStackTrace();
				...
				try {
					// 보조 스트림만 close()해도 기반 스트림도 함께 종료
	 ...
	 * -> BufferedReader 생성 시 보조스트림은 InputStreamReader -> InputStream가 기반 
	 * 
	 */
	public void fileCopy() {
		...
		try {
			...
			// 출력용 스트림 생성
			fos = new FileOutputStream(copy);	 // 출력용 기반 스트림
			bos = new BufferedOutputStream(fos); // 출력용 보조 스트림
