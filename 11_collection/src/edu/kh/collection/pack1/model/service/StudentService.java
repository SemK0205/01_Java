...
public class StudentService {
	...
	public StudentService() { // 기본생성자
		...
		studentList.add(new Student("강아지", 30, "서울시 강남구", 'M', 82));
	 ...
	 * 
	 * - 검색할 이름을 입력받아 stduentList에서 꺼내온 Student 객체의 name 값이 같은지 비교
	 * 
	 ...
	 * 
	 * 문자열 입력받아 studentlist에서 꺼내온 Student 객체의 name 값에 
	 * 포함되는 문자열인지 검사
	 * 
	 ...
	 * List를 정렬하는 방법
	 * 
	 * 방법 1: Comparable 인터페이스 상속받아 compareTo() aptjem wowjddml
	 * Student에 Comparable 인터페이스를 상속받아 오버라이딩한 compareTo()에
	 * 정의한 대로 정렬됨 (나이 오름차순, 나이 내림차순..)
	 * 
	 * 방법 2: Comparator 클래스에 의한 정렬 compare() 사용 (익명 내부 클래스 이용)
	 * 익명 내부 클래스 : 이름이 없는 클래스를 즉석에서 선언해서 
	 * 					  한 번만 사용할 목적으로 작성
	...
	public void sortByName() {
		...
		Collections.sort(studentList, new Comparator<Student>() {
			...
			public int compare(Student o1, Student o2) {
				...
				// (String이 Comparable을 상속받아 재정의해둔 compareTo() 매서드를 이용하는 것)
